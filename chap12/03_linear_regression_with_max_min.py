...
def MinMaxScalar(data):
    numerator = data - np.min(data, axis = 0) # 0은 axis를 의미한다.
    denominator = np.max(data, axis=0) - np.min(data, axis=0) # 가장 큰 격차의 값을 분모로 넣겠다.

    return numerator / (denominator + 1e-7)
# 가장큰 값은 1 가장작은 값은 0으로 표현되게 된다. softmax와 비슷한 역할을 해준다.
# dataset의 단위가 다를경우 가장 많이 이용하는 방법.
